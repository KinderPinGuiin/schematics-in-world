...
public class Model {
    ...
    private ArrayList<ArrayList<Tag>> convertSchematicsToNBT() throws ParserException {
        ...
        // blocks
        ArrayList<BlockData>[][] blockData = this.convertBlocks(blocks, blockEntities,
                                                                size, palette);
    ...
    private void addJigsawInBlocks(HashMap<String, Tag> nbtMap,
                                   JigsawOrientations orientation,
                                   int nElemPalette,
                                   int x, int y, int z
                                   ) {
        int state = orientation.ordinal() + nElemPalette;
        String structName = this.file.getName();
        String pool = "";

    }

    @SuppressWarnings("unchecked")
    private ArrayList<BlockData>[][] convertBlocks(byte[] blockData,
                                                   ArrayList<TagCompound> blockEntities,
                                                   ArrayList<Tag> size,
                                                   TagList palette) {
            ...
            for (TagCompound tc : blockEntities) {
                ...
                for (Tag t : (ArrayList<Tag>) tc.getValue()) {
                    ...
                    if (t.getKey().equals("Id")) {
                        t.setKey("id");
                        nbt.put("id", t);
                    } else if (structX < nbStructX - 1 && (x % MAX_SIZE) == (MAX_SIZE - 1)) {
